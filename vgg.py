...
weight_param    = dict(lr_mult=0.01, decay_mult=0.01)
...
def conv_relu(bottom, ks, nout, stride=1, pad=0, group=1,
    ...
    lrn = L.LRN(norm)
    return L.ReLU(lrn, in_place=True)
...
def minivgg(data, labels=None, train=False, param=weight_param,
    ...
    n.relu1 = conv_relu(n.data, 3, 32, stride=1, pad=1, **conv_kwargs)
    n.relu2 = conv_relu(n.relu1, 3, 32, stride=1, pad=1, **conv_kwargs)
    n.pool1 = max_pool(n.relu2, 2, stride=2, pad=1, train=train)
    
    n.relu3 = conv_relu(n.pool1, 3, 64, stride=1, pad=1, **conv_kwargs)
    n.relu4 = conv_relu(n.relu3, 3, 64, stride=1, pad=1, **conv_kwargs)
    n.pool2 = max_pool(n.relu4, 2, stride=2, pad=1, train=train)
    
    n.relu5 = conv_relu(n.pool2, 3, 128, stride=1, pad=1, **conv_kwargs)
    n.relu6 = conv_relu(n.relu5, 3, 128, stride=1, pad=1, **conv_kwargs)
    n.relu7 = conv_relu(n.relu6, 3, 128, stride=1, pad=1, **conv_kwargs)
    n.relu8 = conv_relu(n.relu7, 3, 128, stride=1, pad=1, **conv_kwargs)
    n.pool3 = max_pool(n.relu8, 2, stride=2, pad=1, train=train)
    
    n.relu9 = conv_relu(n.pool3, 3, 256, stride=1, pad=1, **conv_kwargs)
    n.relu10 = conv_relu(n.relu9, 3, 256, stride=1, pad=1, **conv_kwargs)
    n.relu11 = conv_relu(n.relu10, 3, 256, stride=1, pad=1, **conv_kwargs)
    n.pool4 = max_pool(n.relu11, 2, stride=2, train=train)

    n.relu12 = fc_relu(n.pool4, 1024, param=param)
    n.drop1 = L.Dropout(n.relu12, in_place=True, dropout_ratio=0.5)
    n.relu13 = fc_relu(n.drop1, 1024, param=param)
